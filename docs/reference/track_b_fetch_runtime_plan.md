...
1. **认证不进 adapter。** XIDIAN 登录涉及明文密码、滑块验证码图像匹配、跨子域 CAS 重定向链（每跳携带 ticket 等价物）。这既超出"薄归一化"，又会让 adapter 触碰红线 #1 的等价物。**登录归 ADR-012 的 WebView 会话收割**；fetch/parser adapter 一律假设 **Broker 已注入会话凭证**，只管取数 + 解析。
...
- **多数 XIDIAN 取数 = parser 模式**：登录后就是「带 cookie POST `.do` → 解析 JSON」，请求可静态声明（manifest `requests[]`），核心代取 + 注入 + 脱敏，adapter 纯解析。**不需要 fetch 模式**——优先走更安全的 parser。
- **fetch 模式仅用于运行中途需要 adapter 计算的场景**：反爬挑战（解析内联 JS 算 answer、伪造指纹）、依赖前一步响应体动态拼下一请求。XJT dean 是当前唯一确认的 fetch-only 案例。
- **跨子域 allow**：CAS 链跨 `ids→ehall→yjspt→v8scan`。若某取数确需 fetch 跟随这类链，manifest `network.allow` 须把**每一跳子域**列入（ADR-009 §2.5「每跳须仍在 allow 内」），且中间 `Location`（含 ticket）由 Broker 跟随、永不回交 adapter。
...
| B7 | 录制/回放夹具机制 + 宿主侧单测 + fetch 双跑 | |
| B8 | pattern-based 后置审计（token-pattern 扫描，告警不阻断；随首个 fetch adapter 建初版清单）| |
...
```ts
// CtxFetch 增（仅 fetch 模式、仅 passthrough origin 生效）
...
2. `contract/adapter-sdk/types.d.ts` `CtxFetch` 增 `setEphemeralCookie`（契约改动，红线 #6，向后兼容：纯新增）。
...
§2.8 数值为临时占位。首个 fetch adapter（XJT dean）端到端跑通后，用真实多步握手实测校准并回填 ADR-009 §2.8 正式数值：单请求超时 ~10s / 累计 ~30s / 单次执行最大请求数 ~20（反爬多步可能吃预算，重点验证 20 是否够）。
