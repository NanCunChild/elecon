# Adapter 包与签名机制 · 快速入门（临时文档）

> **这是一份临时的专项讲解**，写给第一次给 elecon 写 adapter 的外部贡献者。
> 它只讲**一件事**：你交出去的那个目录，是怎么变成用户设备上运行的东西的——谁碰过它、
> 谁给它背书、你在这条链上能做什么、不能做什么。
>
...
>
> **时效**：本文写于 bundle digest v2（P0-01）**已定案、实现未落地**的时点。带 `【过渡】`
> 标记的段落会在 v2 落地后失效，届时本文应被删除或重写。角色与流程部分不受影响。
...
> `【过渡】` 当前线上格式是 `elecon-bundle/1`：envelope 里直接内联文件内容，且 digest 的算法
> 不覆盖路径。v2 的动机与规格见 [`adr_018`](../adr/adr_018_adapter_distribution.md) §2.9.1。
> 对你写 adapter 没有影响——变的是打包与验签，不是你写的文件。
