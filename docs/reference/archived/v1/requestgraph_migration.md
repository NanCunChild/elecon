...
> 历史上落实 [V1 ADR-022](../adr/archived/v1/adr_022_request_graph.md)。V2 已退役 requestGraph 双轨，本文仅用于解释 V1 实现。
>
...
- [x] `properties.capabilities.items.properties` **增**：
  ...
  "requestGraph": {
    ...
    "description": "取数请求图声明性（ADR-022）。declarative=manifest requests[] 静态声明、核心代取、adapter 纯解析同步；imperative=adapter 代码 ctx.fetch 自取异步。DEV-Sideload 可任选；DEPLOY 仅运行 official。**required、无 default**（缺字段=校验失败，避免隐式命令式提权）。"
  }
  ```
...
- [x] `fetchTrustPermitted` 闸门触发条件：仍挂在 **imperative 入口**。DEPLOY 保证非 official 永不触达注入；DEV-Sideload 全能力例外可驱动核心使用开发者测试凭证，由编译期 profile 隔离。🔒
...
| 文档 | 动作 |
|---|---|
| `docs/adr/archived/v1/adr_001_contract.md` | ✅ §6「两种调用模式」→「请求图声明性」；§5.1 示例删 mode、cap 加 requestGraph；§5.2 信任档 |
| `docs/adr/archived/v1/adr_002_trust_model.md` | ✅ §2.1 能力表、§2.6 闸门：fetch 模式 → imperative requestGraph；parser ctx → declarative |
| `docs/adr/archived/v1/adr_009_fetch_credential.md` | ✅ 文首术语脚注 + 正文现行术语；`CtxFetch`→`CtxImperative` 叙述 |
| `docs/adr/archived/v1/adr_008_client_runtime.md` | ✅ `async_in_parser` → `async_in_declarative` |
| `docs/adr/archived/v1/adr_013_manifest_credentials.md` | ✅ 示例 mode → requestGraph |
| `docs/adr/archived/v1/adr_015_manifest_login.md` | ✅ 示例同步（若适用） |
| `docs/adr/archived/v1/adr_018_adapter_distribution.md` | ✅ 表中 parser 行 → declarative |
| `docs/adr/archived/v1/adr_014_client_host_fn.md` | ✅ 术语（declarative 路径并列表述） |
