...
> **状态**：**Broker 核心零件 B1–B6 已落地（2026-07-14）**（`server/src/runtime/broker/` + `client/lib/core/broker/` 双端 + golden/集成测试，CI 双跑）；本文落地 [V1 ADR-009 §4](../adr/archived/v1/adr_009_fetch_credential.md) 的运行时清单与 [#3](https://github.com/NanCunChild/elecon/issues/3)，现留作**实现依据 / 历史**，剩余接线由 V2 ADR-000 重新裁定。
