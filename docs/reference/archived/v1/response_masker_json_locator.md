...
> **历史归属**：[V1 ADR-026](../adr/archived/v1/adr_026_response_masker.md) §2.5 / §2.8。
> **相关**：[`response_masker_plan.md`](./response_masker_plan.md) §6、[`response_masker_signoff_checklist.md`](./response_masker_signoff_checklist.md) B1/B2/B5/B6、[V1 ADR-024](../adr/archived/v1/adr_024_build_profile_trust.md)。
...
「至少能在开发产物中溯源 Masker 失败原因」原预备在 [V1 ADR-024](../adr/archived/v1/adr_024_build_profile_trust.md) 落地之后，挂在信任 profile **`DEV`（含侧载 / `ELECON_TRUST_PROFILE` 等编译期 flag 所标定的开发 profile）** 上，**不**绑定传统 `kDebugMode` / `kReleaseMode` 优化等级。V2 已退役该方向。
