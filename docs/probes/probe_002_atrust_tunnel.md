...
> **类型**：go/no-go 探针（gate）。落实 [ADR-000](../adr/adr_000_abstract.md) §5.2 与 [ADR-003](../adr/adr_003_transport.md) §2.4 明列的「三件套探针」：**未过探针不进实现**。
...
> **关联**：ADR-003 §2.4（探针定义）/ §2.5（许可证隔离）/ §2.6（形态 A vs B + 三个开放问题）· [ADR-010](../adr/adr_010_ios_appstore.md) §2.3 · ADR-032（承接本探针结论的子 ADR，待起草）。
