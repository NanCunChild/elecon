...
| **动机** | ① 线上 revocation seq 2 于 2026-09-18T06:31Z 过期，之后 release 门 G2 会拒；② catalog 去端点化后，线上 seq 8 的 catalog 仍带 `url`，签一份干净的才能删字段 |

## 1. 核心仓（elecon）前置

| # | 事项 | 状态 |
|---|---|---|
| 1 | 合并 PR #115（仪式 + 去端点化 + bootstrap 单源）、#116（P3-08 发版门 + TTL 180 天预备） | [ ] owner 签收 |
| 2 | 镜像到 A 仓：`node scripts/mirror-to-adapters-repo.mjs`（vendor 的 `catalog.schema.json` 变为 `url` 可选、validator 带 K3） | [ ] |
| 3 | A 仓改完后 bump `adapters.pin` 到其新 commit，CI 绿 | [ ] |
...
| # | 事项 | 为什么 | 状态 |
|---|---|---|---|
| A1 | `scripts/catalog.mjs`：**不再写 `url`**，去掉 `CATALOG_BASE_URL` 硬要求 | ADR-018 §2.5.1：catalog 只描述文件；vendor 同步后 `catalog:check` 会见到 `url` 已弃用 | [ ] |
| A2 | **版本 bump 只给字节变了的 adapter**：本次若某 adapter 源未动，`adapterVersion` **不要**动 | 台账身份 = `adapterId+adapterVersion` = 一份字节，只记一次；未变字节沿用 seq 8 的记录，发版门 G4 放行。上次全 bump 是因 v1→v2 每份字节都变了 | [ ] |
...
npm run ledger:extract -w .. -- --dist=../dist-9 --key-id=… --public-key-hex=… [--source-commit=… --signed-at=… --signer=… --review-reference=…]
#   ↑ 只把**新身份**（本次 bump 过的 adapter）追加进台账；未变的沿用旧记录
...
npm run dist:export && <上传 ../dist-export 到端点 D>
npm run release:gate -- --online-base=https://elecon.xidian.one/adapters/   # G6：线上 = bootstrap
```
...
| # | 事项 |
|---|---|
| 1 | 删 `url`：`contract/catalog.schema.json` 移除字段、`catalog.dart` 容忍集去掉 `'url'`、validator 去掉 K3；`contract/CHANGELOG.md` 记一条（ADR-018 §2.5.1 已预告） |
| 2 | 整改清单新增执行状态节：seq 9 / 3 参数、台账变化、线上生效时间 |
| 3 | A 仓 mirror 再跑一次（schema 删字段）并 bump `adapters.pin` |
