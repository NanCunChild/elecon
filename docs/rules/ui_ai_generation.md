...
3. **样式全走 design token + 角色枚举，不硬编码。** 颜色 / 间距 / 字号经 token（M3 `ColorScheme.fromSeed` + `ThemeData`），role → 主题样式映射收口到 token 层。必须保证深色 / 浅色 / **高对比度**主题可切换（ADR-004 硬要求），组件不感知具体主题。
...
- [ ] 附 widget 测试：覆盖 adapter 产出 → 标准 schema → 选中正确组件 → 渲染（ADR-004 §端到端验证）；测试数据脱敏。
