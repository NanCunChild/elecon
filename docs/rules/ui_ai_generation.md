...
3. **样式走 design token、文案走 l10n，都不硬编码。** 颜色 / 间距 / 字号经 token（M3 `ColorScheme.fromSeed` + `ThemeData`），role → 主题样式映射收口到 token 层，组件不感知具体主题。ADR-004 §2.4 把「深色 / 浅色 / **高对比度**」列为可切换主题；**（规则层）** 本规则要求生成的组件不得破坏任一主题的可切换性（即一律走 token、不硬编码），使高对比度等无障碍主题始终可用。
   **（规则层）** 文案同理收口：**所有用户可见字符串写在 `client/lib/l10n/*.arb`**，组件里只出现 `AppLocalizations.of(context).<key>`；不可翻译的事实（版本号、官方 URL）放 `client/lib/app_info.dart`。新增显示字段 = 先加 arb 键（zh 为模板 + 同步 en），漏译由 `test/l10n_coverage_test.dart` 抓。语言/主题色等枚举的显示名按 id → l10n 键映射，别把内部 id 露到 UI。
...
- [ ] 用户可见文案全部来自 arb（zh + en 同步），组件内**无**中文/英文字面量；枚举显示名有 l10n 映射。
