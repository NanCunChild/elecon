...
- **适用范围**：`contract/manifest.schema.json` 增 **可选** 顶层 `credentials` 块；`tools/` 校验器据此强制的静态规则；与 parser 模式 `requests[].credential` 的命名空间统一。**不含**：凭证如何注入（ADR-009）、凭证值如何获取/存储（ADR-012）、谁有资格注入（ADR-002）。

---

## 1. 背景（Context）

ADR-009 §2.3 给出了 fetch 模式凭证作用域的 manifest 草图（`credentials.<name>.scope` / `.type`），但**明确声明该草图尚未纳入 `contract/manifest.schema.json`**，须走"独立 issue + PR、与 ADR-001 协调、向后兼容"（红线 #6）。ADR-012 §2.4 定义了核心存储侧的 `CredentialEntry`，其 `ref` 字段注释为"ADR-009 manifest `credentials.<name>` 指向它"——但那个 `credentials.<name>` 在 schema 里**还不存在**。
...
本文把 ADR-009 §2.3 的草图**正式落为 schema**，闭合这个悬空引用，并统一 fetch / parser 两模式的凭证命名空间。**本文只定义"声明契约"，不碰凭证值、不碰注入逻辑**——值与注入分属 ADR-012 / ADR-009。
...
当前 parser 模式用 capability 内 `requests[].credential`（一个字符串）引用凭证，但**无处声明该引用的 scope/type**（ADR-001 §6.2）。本文统一命名空间：

- **parser 模式**：`requests[].credential` 的取值**必须**是 `credentials` 块中声明的某个 `<name>`（`tools/` 强制）。核心代取时据该 `<name>` 的 `type` 注入、据 `scope` 校验请求 URL 在作用域内。
- **fetch 模式**：broker 据请求 URL 匹配 `credentials.<name>.scope` 决定注入（ADR-009 §2.1）。
...
3. **引用闭合**：`requests[].credential`（parser）引用的名必须在 `credentials` 中声明；声明了但**从未被任何请求/scope 使用**的 credential 给 **warning**（非阻断，可能是预声明）。
...
6. **sideload 约束联动**：`trustTier: sideload` + `mode: fetch` 在**官方分发路径**已被拒（ADR-002 §2.6）；其 `credentials` 块在 release 不生效（无 fetch 注入）。dev 侧载-fetch 的 credentials 仅在 debug build 由 dev 测试凭证填充（ADR-002 §2.5），不经 CI 此闸门。
...
| 取向 | 取 | 舍 |
|---|---|---|
| **顶层 `credentials` map + 两模式统一命名空间（建议）** | 单一声明面；fetch/parser 共用；tools 可静态校验 scope；与 ADR-012 `ref` 一一对应 | parser 作者多写一个 `credentials` 块（仅当用到凭证时） |
| 凭证声明内联进每个 `requests[]`（仅 parser 思路延伸） | 局部、就近 | fetch 模式无 `requests[]` 无处放；scope 难表达多步握手；与 ADR-012 `ref` 对应关系散乱 |
...
4. **统一命名空间是对 parser 的轻微收紧**（§2.3）：`requests[].credential` 现要求在 `credentials` 声明。向后兼容靠"无凭证请求不受影响"保证；现存 `school-xidian` 不受影响（已核对：其 request 无 `credential`）。
