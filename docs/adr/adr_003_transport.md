...
- **状态**：已接受（Accepted，2026-06-12）。本文定义传输底座——看到**全部流量**的承重路径（红线 #1）。按 AGENTS.md §1，**AI 不得独自闭环**：由 AI 起草，经人工 review（PR #23）+ 安全检查清单审阅后接受。
- **日期**：2026-06-12（历次修订 2026-06-14 · 06-14b · 07-24 · 08-06 · 09-09 · 09-10）
- **修订流水**：各次修订的动机、取舍与**人工批准记录**已迁至 [`docs/archive/adr_003_revision_log.md`](../archive/adr_003_revision_log.md)；本文正文只承载**当前生效的决策**（同 ADR-002 约定）。
- **状态源约定**：本文只承载决策；**落地与安全签收**的待办态一律只存在于 [`README.md`](./README.md) 索引表与 [`2026_08_review_remediation.md`](../planning/2026_08_review_remediation.md)，正文不重复记录。
...
> 本节为**已接受的决策**（见头部状态）。个别显式标注为「未决」的条目（§2.1.1 通道判据、§2.6 三问）
> 例外，它们各自注明了归属与门槛。
...
- **每请求单通道 + 禁止静默降级（2026-09-09 修订，取代原「一次只有一个 active transport 承载全部流量」）。**
  真正的不变量是：**每一个出站请求，其通道由核心在发出前唯一确定；失败绝不静默降低通道的保护强度**（§2.2 末）。
  （2026-09-10 措辞澄清：原写「绝不回退到更弱的通道」，与 §2.2 降级链第 1 步「`app-tunnel` 失败 →
  `system-vpn` 引导」字面冲突。二者本可自洽——`system-vpn` 与 `app-tunnel` 同为隧道、保护强度相当，
  换的是实现不是强度。**唯一的降级终点是只读公开缓存**，即拒绝而非降格。语义未变。）
...
| **`app-tunnel`** | **App 内原生隧道**（Hermes 属此，形态 B） | **首发 Android**；iOS **本轮不编入**（**非因 GPL/entitlement——那两道已解除——而是 App Store 指南 5.2.2 未评估**，见 §2.5）；OHOS/桌面按优先级 | **编译期编入 + 远程开关**（§2.3），最高信任档 | **否**（标的自有 MIT，见 §2.5） |
...
>
> **第 1 步不是「降级」，是同强度换实现**：`system-vpn` 与 `app-tunnel` 同为隧道，保护强度相当，
> 差别在谁承载隧道（系统/第三方 App vs elecon 二进制内）。**真正的降级只有一处——退到只读公开缓存**，
> 那是**拒绝访问私密数据**，不是用更弱的通道去访问它。§2.1「失败绝不静默降低通道的保护强度」与本链一致。
...
- **kill-switch = 远程开关，不是代码吊销。** 急性事件下把某个 transport 档**远程禁用**（本节第 2 条），核心按 §2.2 降级链退到 `system-vpn` / `direct`（fail-safe）。它纳入 [`adr_002`](./adr_002_trust_model.md) §2.4 的**远端治理面**，但**不是** §2.4 的 bundle 吊销清单——被禁用的代码仍在二进制里，只是不被使用；要让它从产物里消失只能发新版。**「吊销某个 transport 版本」对编译进二进制的模块不成立**（见本节第 1 条）。

### 2.4 atrust VPN 复刻的接入 = 三件套探针先行（gate）

延续 ADR-000 §5.2「不焊死 + 开工前探针」，**未过探针不进实现**。**三件套已于 2026-09-09 全部完成，判定 go（附条件）**：

1. ~~**许可证探针**~~ **✅ 结论：自有 MIT**（Hermes；`LICENSE` 全文 + `Cargo.toml` 声明，2026-09-09 核实），依赖树零 GPL 系。
2. ~~**协议模式探针**~~ **✅ 结论：纯隧道封装，不终止 app TLS**。标的属**形态 B**（用户态网络栈，可移植）；西电真机三个目标 HTTP 200 且**目标证书完整校验**（若被本地拆 TLS，证书链会是隧道自签而非目标域名），故不触红线 #1、无需走 §3.4 的单独评审。见 [`probe_002`](../probes/probe_002_atrust_tunnel.md)。
3. ~~**iOS 可行性探针**~~ **⬜ 结论：本轮 N/A，不予认定**。原先的**两道**封锁确已解除——MIT 消解分发不相容（§2.5）、**形态 B** 不用 `NetworkExtension`/不需 entitlement/不触指南 5.4（§2.6）——故「iOS 因 GPL + entitlement 不可上」这个**旧结论作废**。
   **但「封锁解除」≠「可上」**：还有**第三道门**——App Store 指南 **5.2.2**（逆向私有协议连接第三方即学校网关的合规性）**从未评估**，[`probe_002`](../probes/probe_002_atrust_tunnel.md) §3 明确**不予认定 iOS 可上架**。故当前决策是 **iOS 本轮不编入**（与 [`adr_010`](./adr_010_ios_appstore.md) §2.3、[`adr_032`](./adr_032_app_tunnel_embedding.md) §2.8 一致）。
...
叠加 §2.6 已为**形态 B** 解除 NetworkExtension / entitlement / 指南 5.4 的封锁，**iOS 原有的两道封锁
均已解除**。

> **⚠ 但两道门解除 ≠ iOS 可上（2026-09-10 更正）。** 本节 2026-09-09 的初稿据此写成「iOS 可上 /
> 全平台可编入」，**那是过度解读**：owner 当日的决策原文是「iOS 平台**可以重新纳入考虑**」，而
> 「考虑」与「可上」之间还隔着**第三道门**——App Store 指南 **5.2.2**（逆向私有协议连接第三方
> 即学校网关的合规性），它**从未被评估**。三处独立来源对此一致：
> [`probe_002`](../probes/probe_002_atrust_tunnel.md) §3「未认定 iOS 可上架……本探针不予认定」、
> [`adr_010`](./adr_010_ios_appstore.md) §2.3「当前决策仍是 iOS 不编入 app-tunnel，首发仅 Android」、
> [`adr_032`](./adr_032_app_tunnel_embedding.md) §2.8「iOS ❌ 不编入——非因 GPL，而是指南 5.2.2 未评估」。
> 初稿还声称「§2.2 矩阵与 adr_010 §2.3 已同步」——**该同步从未发生**，ADR-010 §2.3 自 2026-08-06 起
> 一直写的是「iOS 不编入」。§2.2 矩阵与 §2.4 探针 3 已随本次更正改回。
>
> **当前决策：`app-tunnel` 首发仅 Android，iOS 本轮不编入。** 若日后要上 iOS，须**单独做 5.2.2 评估**
> 并回写 ADR-010（其 §4 复评触发器已被触发），本文不预先批准。
...
**iOS 判定修订**：早先"iOS 默认不编入"的理由是 **NetworkExtension + entitlement + 指南 5.4**，这三项**仅适用于形态 A**。**形态 B 不触这三项**——它不建 NetworkExtension，从 iOS 视角是普通出站 TLS app。唯一功能代价：普通 app 后台被挂起 → 无 always-on 隧道；但 elecon 的 on-demand 取数（开 app→连隧道→拉数据→拆）**不需要 always-on**，可接受。

> **本条只解除 5.4 这一道门，不构成「iOS 可上」的结论。** 尚有**指南 5.2.2**（私有协议逆向连校网关的第三方服务合规）**未评估**——它不是"残余小风险"，而是一道**未走的门**（[`probe_002`](../probes/probe_002_atrust_tunnel.md) §3 不予认定）。当前决策为 **iOS 本轮不编入**，见 §2.5 与 §2.2 矩阵。拟议缓解（备学校认可学生 VPN 使用的材料）留待该评估时使用。

**开放问题（显式未决）已于 2026-08-06 整体移交 [`adr_032`](./adr_032_app_tunnel_embedding.md)。**

三问——① 是否净室重写 aTrust 北向协议；② 若净室，嵌入形态用 FFI 还是本地回环；③ 净室的 AI 特有
风险（模型权重可能已训练过公开源码）——**正文不再复述**，以免与承接方各存一份副本而漂移
（2026-09-10 去重：此处曾保留一份，其中「直接接入 AGPLv3 的 zju-connect」在 §2.5 定标 Hermes 后
已非活选项，属过时表述）。三问的现行表述与结论**一律以 ADR-032 §2.1/§2.2/§2.6 为准**。

**门槛不变**：ADR-032 仍为 **Proposed**，其 §5 的五项未决（含表达性选择复现审查 U-1、安全清单与
人工审签 U-5）未闭前，**不得合并任何隧道实现代码**。事实基础见
[`probe_002_atrust_tunnel.md`](../probes/probe_002_atrust_tunnel.md)（判定 go 附条件）。
...
5. **跨平台能力不对称**（iOS 最弱）带来产品文案/预期管理成本。**2026-09-09 收窄、2026-09-10 更正**：MIT 标的 + 形态 B 解除了 iOS 的 GPL 与 entitlement/5.4 两道封锁（§2.5/§2.6），**但指南 5.2.2 未评估，iOS 本轮仍不编入 `app-tunnel`**（首发 Android）。故当前 iOS 与 Android 的差距**不止**「无 always-on」一项，而是**整档不可用**，iOS 只有 `direct` + `system-vpn`。5.2.2 评估通过后，差距才收窄到「无 always-on 后台隧道」——而那一项 on-demand 取数不需要，影响有限。Windows 仍受限（ADR-032 §4.7：UDS 不可用）。
...
7. **待确认（不阻塞本 ADR）：iOS Personal VPN entitlement 可得性。** `system-vpn` 档在 iOS 依赖 **Personal VPN entitlement**（`NEVPNManager`）。其门槛远低于 Network Extension，但仍是一项 entitlement 依赖，且 [ADR-010](./adr_010_ios_appstore.md) 未就此评估。**取向**：本 ADR 已在标注「待确认」的前提下接受——若该 entitlement 因审核策略不可得，iOS 的 `system-vpn` 引导退化为"提示用户在系统设置自行配置 VPN"（纯引导、零 entitlement），不阻塞 `direct` 档与本 ADR 主体。须在 iOS 上架评估（§4 / ADR-010 §3.3）时一并确认。
...
- **transport 抽象接口（核心侧）**：lifecycle/status/routing 窄接口；**每请求单通道**（§2.1）+ 运行时切换 + 降级链；**TLS 不终止**不变量落为代码约束。客户端与（如适用）`server/src/campus` 对齐。
...
- **测试**：transport 状态机/降级链单测；**TLS-不终止**断言；**远程开关正反例**（禁用某档 → 按降级链退档；~~签名/吊销正反例~~ 已随 §2.3 作废）；**无侧载入口断言**；不在 UI 线程阻塞（红线 #7 同源精神，原生侧勿阻塞主线程）。
