...
- **禁止预取校验**：客户端**不得**在用户点击前对外链 URL 发起任何探测/预检请求。预取 = 替用户在其意图之前自动访问 adapter 选定地址；若该 URL 落在 `network.allow` + 凭证注入作用域内，预取可能**自动携带凭证**打出去（轻量 SSRF / 时序泄露）。新鲜度由数据信封（data envelope）TTL 承载（ADR-001 §3.3），外链一律按**尽力而为**对待。
