...
import { FakeTransport, readText, resp, resolveRepoRoot, runMain } from "./__testutils__/smoke-utils.js";
...
const adapterDir = `${publicRoot.replace(/\/$/, "")}/adapters/school-fudan`;

async function main(): Promise<void> {
  const source = readText(`${adapterDir}/index.js`);
  const fixture = JSON.parse(readText(`${adapterDir}/fixtures/notice.list.json`));
  const transport = new FakeTransport([resp({ status: 200, headers: { "content-type": "text/html" }, body: fixture.responses.page.body })]);
  ...
  const { data } = await runFetchAdapter(
    ...
    { trust: TrustedAdapterContext.devSideload(), view, resolver: { async get() { return null; } }, transport },
  );
